...
### 12. 関数の穴（reactive な子）はテキスト限定 [割り切り]

`() => count.value` の穴はテキストノードの更新のみ。関数が Node を返しても文字列化される。
（Node の差し替えは `For` / `Show` の担当、という設計。）

**対応案**: 基本は現状維持でよい。汎用化するなら「戻り値が Node なら前回ノードと
replaceWith で差し替え、内部を createRoot で包んで dispose する」ミニ版 Show を
`appendChild` / `toNode` に入れる。ただし `Show` と役割が重複するので、
README に「Node を返す関数は不可、For / Show を使う」と明記する方が筋が良い。
...
`html` にも同様に当てはまる。`setAttr` が h.ts と html.ts に重複定義されているので、
対応の際は共通化（`dom-shared.ts` 等に抽出）してから入れると二度手間にならない。

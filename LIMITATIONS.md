...
### 12. 関数の穴（reactive な子）は更新のたびに範囲を全部作り直す [割り切り]

`() => ...` の子穴は `Node` / 配列も返せる（範囲再描画。`node.ts` の `toNode` で h / html 共通）。
コメント2つで挟んだ範囲を作り、依存が変わるたびにその範囲を丸ごと作り直して入れ替える
（プリミティブだけはテキストノードを使い回す fast path あり）。差分は取らないので、
状態保持が要る場面はコンポーネント側に委ねる住み分けになっている:

- リストの並べ替え・挿入で行の状態（input・フォーカス・行ローカル signal）を保ちたい → `For`
- 条件が真のあいだ部分木を据え置きたい（真偽が変わったときだけ作り直したい）→ `Show`

**対応案**: 現状維持でよい。範囲再描画は「素の `.map` / 三項演算子で書ける」手軽さと
引き換えの割り切りなので、`For` / `Show` との使い分けを README に明記しておくと迷わない
（単純な boolean の出し入れは関数穴で足り、`Show` は真偽安定・状態保持が要る場面に絞る）。

### 13. `tags` の型が `Record<string, TagBuilder>` [未実装]
...
### 14. イベントオプション（capture / once / passive）を渡せない [未実装]
...
### 17. 並べ替えが毎回「全ノードを insertBefore」 [未実装]
...
### 18. 同じ key で中身が新しいオブジェクトに変わっても行が更新されない [未実装]
...
### 19. `render` に index が渡らない [未実装]

順位表示など index 依存の描画ができない。

**対応案**: #18 と同様に `Signal<number>` を第2引数で渡し、並べ替え時に更新する。

---

## show.ts

### 20. `render` に when の値が渡らない [未実装]
...
### 21. disconnect 時の `replaceChildren()` が host の子を全部消す [未実装に近い問題]
...
### 23. `attr()` が `string | null` のみ・複雑なデータを渡す口がない [未実装]
...
| 優先 | 項目 | 理由 |
|---|---|---|
| 高 | #21 replaceChildren の巻き添え | 実害のあるバグに近い。修正も数行 |
| 高 | #8 プロパティ設定（input.value 等） | フォームを書くと即座に踏む |
| 高 | #18 For の同 key 新オブジェクト | immutable 更新が静かに壊れる |
| 中 | #1/#2 memo の lazy 化とグリッチ解消 | コアの質が上がるが書き換え規模が大きい |
| 中 | #17 For の同位置スキップ | 数行で大半の無駄が消える |
| 中 | #3 untrack / #4 equals / #28 sideEffects | 小さく足せて効果が確実 |
| 低 | #9 SVG / #22 shadow DOM / #23 prop | 必要になったときに |
| 低 | #25〜#27, #29 インフラ整備 | 機能とは独立にいつでも |

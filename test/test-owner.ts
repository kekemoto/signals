...
test("owner: 親再実行で前回の子が畳まれる", () => {
  ...
  setOuter(1); // 親再実行 → 前回の子を畳んで新しい子を作る
  assert.equal(childRuns, 2, "親再実行で新しい子");
  setInner(1); // 子が反応。リークしていれば古い子も走り +2 になる
...
test("cached: オーナーがないと dev 警告（createRoot / effect 内では出ない）", () => {
  ...
  try {
    cached(() => a()); // オーナーなし（孤児）→ 警告
    assert.equal(warns, dev ? 1 : 0, "オーナーなしで警告");
    createRoot((dispose) => {
      cached(() => a()); // createRoot がオーナー → 警告なし
      dispose();
    });
    effect(() => {
      cached(() => a()); // 親 effect がオーナー → 警告なし
    })();
...
test("createRoot は親の所有ツリーに繋がらない（独立スコープ）", () => {
  ...
  setOuter(1); // 親再実行。root は独立なので前回の中身は畳まれない
...
test("createRoot 直下での signal 読みは追跡されない", () => {
  ...
  createRoot(() => {
    rootRuns++;
    s(); // root 直下で読む（effect ではない）
  });
  setS(1); // 追跡されていなければ rootRuns は増えない
...
test("onError: ハンドラが無ければ従来どおり投げ直す", () => {
  const [s, setS] = signal(0);
  effect(() => {
    if (s() === 1) throw new Error("unhandled");
  });
  assert.throws(
    () => {
      setS(1);
...
test("onError: 例外を捕捉しても同じ世代の他の effect は走る", () => {
  ...
  setS(1); // 両方 re-run。a は throw されるが捕捉、b は走る

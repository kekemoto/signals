...
const { runHydration, hydrate, HYDRATE_ATTR } = await import("../src/hydration.js");
...
// === stage 4: 公開エントリ hydrate + dispose ===
test("hydrate: 公開エントリで html を採用し、dispose で配線を畳む", () => {
  const count = signal(0);
  const server = emit`<span>n=${() => count.value}</span>`;
  assert.equal(server, "<span>n=<!--hole-->0<!--/hole--></span>", "前提: emit 出力");
  const container = serve(server);
  const span = container.querySelector("span")!;

  let dispose!: () => void;
  const mutations = countChildListMutations(container, () => {
    dispose = hydrate(container, () => html`<span>n=${() => count.value}</span>`);
  });
  assert.equal(container.querySelector("span"), span, "ルートを採用（===）");
  assert.equal(mutations, 0, "hydrate 中の childList 変化は 0");

  count.value = 1;
  assert.equal(span.textContent, "n=1", "採用後に signal 更新が反映される");
  dispose();
  count.value = 2;
  assert.equal(span.textContent, "n=1", "dispose 後は effect が走らない（配線を畳んだ）");
});

// === stage 4: defineElement の adopt モード ===
test("defineElement: マーカー付き host の既存子を採用して配線する", () => {
  defineElement("x-hydr-counter", () => {
    const count = signal(5);
    return html`<button onClick=${() => count.value++}>n: ${() => count.value}</button>`;
  });

  // サーバ描画相当: host にマーカー、中身は emit 出力と同形（reactive 子は開閉ペア）。
  const host = document.createElement("x-hydr-counter");
  host.setAttribute(HYDRATE_ATTR, "");
  host.innerHTML = "<button>n: <!--hole-->5<!--/hole--></button>";
  const button = host.querySelector("button")!;
  const textNode = button.childNodes[2]; // "n: " | <!--hole--> | "5" | <!--/hole-->
  assert.equal(textNode.textContent, "5", "前提: サーバ初期テキスト");

  const container = document.createElement("div");
  container.append(host);
  // connectedCallback は document へ繋いだ瞬間に同期で走る。その間の childList 変化を数える。
  const mutations = countChildListMutations(container, () => document.body.append(container));

  assert.equal(mutations, 0, "adopt では childList を変えない（作り直さない）");
  assert.equal(host.querySelector("button"), button, "既存 button を採用（===）");
  assert.equal(host.children.length, 1, "setup 出力を二重に append していない");
  assert.equal(host.hasAttribute(HYDRATE_ATTR), false, "採用後にマーカーを strip する");

  // 採用後の reactivity: click でハンドラ発火・テキスト更新（テキストノードは使い回し）。
  button.click();
  assert.equal(button.textContent, "n: 6", "採用後に click→signal 更新が反映される");
  assert.equal(button.childNodes[2], textNode, "プリミティブ更新はテキストを使い回す（===）");
});

// === stage 4: マーカーなし host は従来どおり新規生成する ===
test("defineElement: マーカーなしなら従来どおり新規生成する（adopt しない）", () => {
  defineElement("x-plain-build", () => {
    const msg = signal("hi");
    return html`<p>${() => msg.value}</p>`;
  });

  const host = document.createElement("x-plain-build");
  // マーカーなしで既存子（=利用者の slot 入力扱い）を置いておく。
  host.innerHTML = "<span>leftover</span>";
  document.body.append(host);

  // 既存子は退避され、setup の出力（<p>hi</p>）が描画される。
  assert.equal(host.querySelector("span"), null, "マーカーなしでは既存子を採用しない（退避）");
  assert.equal(host.querySelector("p")?.textContent, "hi", "setup の出力が新規描画される");
});

// === #54-A: For を html テンプレに入れ子にしてもラッパ要素の穴が実ノードへ配線される ===
// 旧実装では eager 評価の For が共有カーソルを先食いし、html がルートを claim できず捨てる
// クローンへ配線していた（cls.value を変えても実 DOM の class が変わらない）。
test("#54-A: For を html テンプレに入れ子にしてもラッパ属性と行を両方採用する", () => {
  const cls = signal("box");
  const items = signal([
    { id: 1, text: "one" },
    { id: 2, text: "two" },
  ...
  assert.equal(root, ul, "ラッパ <ul> を作り直さず採用する（===）");
  assert.equal(mutations, 0, "入れ子 For の採用でも childList を変えない");
  assert.equal(ul.children[0], li1, "1行目を採用（===）");
  assert.equal(ul.children[1], li2, "2行目を採用（===）");

  // ラッパの属性穴が「実ノード」に配線されている（旧バグでは捨てクローンに配線されていた）。
  cls.value = "panel";
  assert.equal(ul.getAttribute("class"), "panel", "ラッパ属性の穴が実 <ul> に配線される");
...
// === #54-A': Show を html テンプレに入れ子にしても同様に採用できる ===
test("#54-A': Show を html テンプレに入れ子にしてもラッパ属性と中身を採用する", () => {
  const attr = signal("a");
  const on = signal(true);
  const container = serve(
    '<section data-x="a"><!--show--><p><!--hole-->yes<!--/hole--></p><!--/show--></section>',
  );
  ...
  const mutations = countChildListMutations(container, () => {
    root = runHydration(
      container,
      () => html`<section data-x=${attr}>${Show(on, () => html`<p>${() => "yes"}</p>`)}</section>`,
    );
  });
  assert.equal(root, section, "ラッパ <section> を採用（===）");
  assert.equal(mutations, 0, "入れ子 Show の採用でも childList を変えない");
  assert.equal(section.querySelector("p"), p, "Show の中身を採用（===）");

  attr.value = "b";
  assert.equal(section.getAttribute("data-x"), "b", "ラッパ属性の穴が実 <section> に配線される");
  on.value = false;
  assert.equal(section.querySelector("p"), null, "Show を false にすると中身が消える");
});

// === #54-B: 複数ルート（fragment）テンプレートは全ルートが配線される ===
// 旧実装は claimRoot で最初のルートだけ掴み、2つ目以降のルートの穴を配線しなかった。
test("#54-B: 複数ルート（fragment）テンプレートの全ルートを採用・配線する", () => {
  const a = signal("A");
  const b = signal("B");
  const container = serve("<i><!--hole-->A<!--/hole--></i><b><!--hole-->B<!--/hole--></b>");
  const i = container.querySelector("i")!;
  const bEl = container.querySelector("b")!;

  const mutations = countChildListMutations(container, () => {
    runHydration(container, () => html`<i>${() => a.value}</i><b>${() => b.value}</b>`);
  });
  assert.equal(mutations, 0, "複数ルートの採用でも childList を変えない");
  assert.equal(container.querySelector("i"), i, "1つ目のルートを採用（===）");
  assert.equal(container.querySelector("b"), bEl, "2つ目のルートを採用（===）");

  a.value = "X";
  b.value = "Y";
  assert.equal(i.textContent, "X", "1つ目のルートの穴が配線される");
  assert.equal(bEl.textContent, "Y", "2つ目のルートの穴も配線される（旧実装では未配線）");
});

// === #54-C: サーバ DOM の余分な要素で以降の属性穴がずれない（構造的カーソル） ===
// テンプレが <x-card> を葉として持つのに、サーバ DOM では中身（投影済み light DOM）がある。
// 旧フラット文書順位モデルでは <x-card> の中の要素が順位を1つ食い、後続の属性穴が別要素へずれた。
test("#54-C: 葉要素の中にサーバ投影があっても後続の属性穴がずれない", () => {
  const cls = signal("box");
  // テンプレ: <div><x-card></x-card><b class=${cls}>hi</b></div>
  // サーバ DOM: x-card の中に投影済みの <span> がある。
  const container = serve(
    '<div><x-card><span>projected</span></x-card><b class="box">hi</b></div>',
  );
  const bEl = container.querySelector("b")!;
  const span = container.querySelector("span")!;

  runHydration(container, () => html`<div><x-card></x-card><b class=${cls}>hi</b></div>`);

  cls.value = "panel";
  assert.equal(bEl.getAttribute("class"), "panel", "属性穴は意図どおり <b> に配線される");
  assert.equal(
    span.hasAttribute("class"),
    false,
    "投影済みの <span> へ誤配線されない（ドリフトしない）",
  );
});

// === #54: 入れ子の html 断片（${html`...`}）も採用できる ===
test("#54: 子穴に入れ子の html 断片を入れても採用・配線できる", () => {
  const x = signal("hi");
  const container = serve("<div>before<span><!--hole-->hi<!--/hole--></span>after</div>");
  const div = container.querySelector("div")!;
  const span = container.querySelector("span")!;
  ...
  const mutations = countChildListMutations(container, () => {
    root = runHydration(
      container,
      () => html`<div>before${html`<span>${() => x.value}</span>`}after</div>`,
    );
  });
  assert.equal(root, div, "外側 <div> を採用（===）");
  assert.equal(mutations, 0, "入れ子 html の採用でも childList を変えない");
  assert.equal(div.querySelector("span"), span, "入れ子 html のルートも採用（===）");

  x.value = "bye";
  assert.equal(span.textContent, "bye", "入れ子 html の reactive 子が配線される");
});

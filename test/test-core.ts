...
import { batch, effect, isSignal, signal, untrack } from "../src/reactive.js";
...
test("キャッシュする派生は signal + effect で組める（共有・cutoff）", () => {
  const w = signal(2),
    h = signal(3);
  // 「キャッシュ＋共有＋value-cutoff」が要るときの定番パターン。
  const area = signal(w.peek() * h.peek());
  effect(() => {
    area.value = w.value * h.value; // setter の Object.is が cutoff を担う
  });
  let downstream = 0;
  effect(() => {
    area.value;
  ...
  assert.equal(area.value, 6, "派生セル 初期値");
  ...
  assert.equal(area.value, 6, "面積は同じ");
  assert.equal(downstream, 1, "結果不変なら下流据え置き（cutoff）");
  // 面積が変わる → 下流も走る
  h.value = 2;
  assert.equal(area.value, 12, "面積が変化");

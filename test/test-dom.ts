...
test("html: `.checked` 穴はプロパティ false で外れる", () => {
  ...
  const el = html`<input type="checkbox" .checked=${() => on.value}>` as HTMLInputElement;
...
test("class: reactive オブジェクトで反転を追従", () => {
  const on = signal(false);
  const el = html`<div class=${() => ({ active: on.value, base: true })}></div>` as HTMLElement;
...
test("setAttr: aria-*/data-* も真偽値は全キー共通（false=削除で付け外しできる）", () => {
  ...
  const on = signal(true);
  const el = html`<div data-on=${() => on.value}></div>` as HTMLElement;
...
test("toNode: 子の true / false はどちらも非表示", () => {
  ...
  const el = html`<span>[${() => flag.value}]</span>` as HTMLElement;
...
test("For: render に index が渡る（#18）", () => {
  ...
  el.append(
    html`<ul>${For(
      items,
      (i) => i.id,
      (item, index) => html`<li data-id=${item().id}>${() => `${index() + 1}:${item().id}`}</li>`,
    )}</ul>`,
  );
  const cell = (id: string) => el.querySelector(`li[data-id="${id}"]`)!.textContent;
  assert.ok(
...
test("defineElement: upgrade 前のプロパティ代入を初期値として拾う", () => {
  ...
  defineElement("x-early", ({ prop }) => {
    ...
    return html`<span>${() => String(label.value)}</span>`;
  });

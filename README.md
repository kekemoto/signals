...
props は `setAttribute` で属性として渡る（値は文字列化される）。Custom Element に
オブジェクト/配列などリッチなデータを**プロパティ**で渡したいときは、返ってきた要素に
直接書き込む。reactive にしたいなら `effect` で包むだけ（新しい API は要らない）。

```js
const el = h("my-list");
el.items = [{ name: "a" }, { name: "b" }];          // プロパティは文字列化されない
effect(() => { el.selected = selected.value; });    // signal に追従させたいとき
```

...
サーバ（または SSG / ビルド時プリレンダー）が出した HTML を、クライアントが**作り直さずに
採用（adopt）**して reactive を配線する。focus・入力値・スクロール等の DOM 状態と接続済みの
`effect` を壊さない「状態保存型（ノード保存）」のハイドレーション。対象は `` html`...` ``
（`h` / `tags` は現状スコープ外）。

仕組みは3つの部品でできている。

- **`emit`** — `html` と同じタグ付きテンプレートを受け取り、**値を埋めた HTML 文字列**を返す
  純粋関数（`document` / `customElements` に触れないので Node などのサーバから import できる）。
  `effect` は張らず、関数穴は1回呼ぶ・signal は `.value` を1回読むだけ（＝計算後の初期値を埋める）。
  reactive な子穴は `<!--hole-->…<!--/hole-->` の開閉ペアで囲み、イベント / `ref` /
  プロパティ（`.foo`）穴は属性を吐かない。本文 `&` `<` ・属性値 `&` `"` を XSS 対策でエスケープする。
- **`hydrate(container, fn)`** — `container` の既存子（サーバが出した DOM）を採用範囲として
  `fn`（`html` を返すテンプレ）を実行し、reactive を配線する。中の `html` / `For` / `Show` は
  新規生成せず既存ノードを claim して使い回す。戻り値の `dispose` を呼べば張った `effect` を畳める。
- **`defineElement` の adopt モード** — host に `data-hydrate`（`HYDRATE_ATTR`）が付いていれば、
  `connectedCallback` は既存の light DOM（サーバが出した setup の出力）を採用して配線する
  （新規生成・再 append しない）。採用後はマーカー属性を strip する。
...
const markup = emit`<span>count: ${() => count.value}</span>`;
// → '<span>count: <!--hole-->7<!--/hole--></span>'  これを HTML に埋めて送る
```

```js
// --- クライアント（同じテンプレ・同じ初期値で配線だけ張る）---
import { hydrate } from "@kekemoto/signals";
import { html } from "@kekemoto/signals/html";

const count = signal(7); // サーバと一致する初期値（mismatch を避ける）
const dispose = hydrate(document.querySelector("#app"), () =>
  html`<span>count: ${() => count.value}</span>`,
);
// 既存の <span> をそのまま使い、effect だけ張る（作り直さない）
count.value = 42; // テキストが更新される
```

Custom Element 境界で起動する（アイランド的）なら、host に `data-hydrate` を付けて送り、
`defineElement` の adopt モードに任せる。`connectedCallback` が既存子を採用する。

```html
<!-- サーバ出力: host にマーカー、中身は emit 出力と同形 -->
<x-counter data-hydrate>
  <button>n: <!--hole-->5<!--/hole--></button>
</x-counter>
```
...
// 接続時に既存 <button> を採用し、click / テキスト更新を配線（作り直さない）
```

> **現状の制約**:
> - `emit` が文字列化できるのは**プリミティブな子だけ**。`Node` の子・ネストした `` html`...` ``
>   は throw し、`For` / `Show` はシリアライズできない（リスト / 条件を含む SSR はサーバ HTML を
>   手組みする必要がある）。一方、ハイドレーション側（adopt）は `For` / `Show` の
>   `<!--for-->` / `<!--show-->` 開閉ペアを採用できる。
> - `defineElement` の adopt は **slot を使わない要素**（props / signal だけで中身を組むアイランド）が
>   対象。採用中の `ctx.slot()` は空を返す。
> - 単純 props（文字列属性）を超えるリッチな初期データの直列化（JSON など）は未対応。
>
> 設計の全体像・実装の対応関係・残課題は [`docs/ssr-hydration-plan.md`](docs/ssr-hydration-plan.md) を参照。
...
```
src/    reactive.ts / store.ts / node.ts / h.ts / tags.ts / html.ts / for.ts / show.ts /
        element.ts / emit.ts / hydration.ts / index.ts
test/   test-core.ts / test-owner.ts / test-emit.ts / test-dom.ts / test-hydrate.ts
```
...
```bash
npm test       # build してから core / owner / emit テストを実行（jsdom 不要）
npm run test:dom  # build してから DOM / hydrate テストを実行（jsdom が必要）

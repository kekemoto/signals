...
挙動もそのまま。`"closed"` では `el.shadowRoot` が `null` になる点だけ Web 標準どおり異なる。

> Lit は `createRenderRoot()` で renderRoot（shadowRoot か host）を差し替えるだけで描画を共通化
> するが、slot だけはネイティブ機能ゆえ light DOM で使えない。このライブラリは slot も自前の
> 静的投影なので、light/shadow で投影まで含めて挙動を揃えている。

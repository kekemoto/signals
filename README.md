...
`setCount` するだけでボタンのテキストが書き換わる。仮想 DOM の差分も全体再レンダーも
なく、`${count}` の箇所だけがピンポイントで更新される（細粒度更新）。`count` は読みの
accessor（関数）なので、穴へは `() =>` で包まず `${count}` と直接渡せる。
...
値を持つリアクティブセル。`[読み, 書き]` のタプルを返す。読みは accessor（`count()` で
現在値を返し、`effect` 内なら依存登録する）、書きは setter（`setCount(v)`）。追跡せずに
今の値だけ読みたいときは `untrack(count)`。
...
> reactive な値（signal の読み・派生・`cached`）は**すべて関数**に揃う。だから穴へは
> `${count}` と直接渡せ（`reactive ⇔ 関数` の単一ルール）、合成式は `${() => a() + b()}`
> のように関数で包む。「読むなら呼ぶ・穴へは渡す」だけで全種類を説明できる。
...
const [a, setA] = signal(1), [b, setB] = signal(2);
...
このライブラリに `memo` / `computed` はない。派生値は**普通の関数**で書く。関数は中間ノードを作らず、読まれた瞬間に最新値を計算するので、lazy（読まれなければ計算しない）でグリッチも起きない。signal の読みも関数なので、派生と読み口が一律 `()` に揃う。

```js
import { signal, effect } from "@kekemoto/signals";

const [a, setA] = signal(3), [b] = signal(4);
...
const [w, setW] = signal(2), [h] = signal(3);

// const area = () => w() * h();       // 素の派生（出発点）
const area = cached(() => w() * h());  // ↑ をホット化（呼び出し側 area() は無変更）

effect(() => console.log(area())); // → 6
effect(() => console.log(area())); // 再計算なし・キャッシュを共有
setW(6);                           // → 両 effect に 18（=6*3）が流れる。計算は1回だけ
```
...
createRoot(() => {
  ...
  effect(() => {
    if (userId() < 0) throw new Error("不正な userId");
    // ... 描画など、投げうる処理 ...
  });
});

setUserId(-1); // effect の例外は↑のハンドラへ届く（書いた側へは飛ばない）
```
...
ときに使う（読んだ signal が変わっても再実行されない）。accessor をそのまま渡せる
（`untrack(count)`）ほか、複数の signal を素通しで読むなら関数で包んで渡す。

```js
import { signal, effect, untrack } from "@kekemoto/signals";

const [value, setValue] = signal(0);
const [verbose, setVerbose] = signal(true);

effect(() => {
  // value が変わったときだけログ。verbose の切り替えでは再実行させたくない
  if (untrack(verbose)) console.log("value:", value());
});

setVerbose(false); // 再実行されない（追跡していない）
setValue(1);       // 再実行される（追跡している）
```
...
> **reactive な穴の渡し方** — `` html`...` `` の穴に **関数**を渡すと reactive になる
> （`reactive ⇔ 関数` の単一ルール）。signal の読みは accessor（`count` がそのまま関数）
> なので `${count}` で直接渡せ、複数の値を組み合わせる派生は `() => a() + b()` のように
> 関数で包む（`${...}` はその場で評価されるため、合成式は関数が必須）。

### `` html`...` ``

タグ付きテンプレートリテラルで reactive な DOM を作る（lit / htm 風）。
静的な構造は `<template>` で一度だけパースし、`${...}` の穴だけを配線する。
関数の穴は reactive（属性・子）になり、`onXxx=${fn}` はイベント、
...
> html`<x-list .items=${items}></x-list>`;   // el.items = items()（accessor 直渡し）
> ```
>
> フォーム要素の **`value` / `checked` / `selected`** は、属性だと「初期値」しか変えられず
> ユーザー入力後は属性とプロパティが乖離する。signal の変更を常に画面へ反映したいなら
> プロパティとして渡す（穴と同じく **accessor 直渡し**でよい）:
>
> ```js
> html`<input .value=${text}>`;            // accessor 直渡し。入力後も signal の変更が反映される
> html`<input value=${"既定値"}>`;         // 初期値だけ（入力後は更新しても効かない）
> ```
>
> プロパティは値を丸めず素のまま代入する。`null` を空にしたいなら
> `.value=${() => text() ?? ""}` のように呼び出し側で処理する（ここは派生なので関数で包む）。
...
> CSS カスタムプロパティ `--gap` も書ける）。関数で包めば reactive になり、丸ごと置換
...
> html`<div class=${() => ({ active: on() })}></div>`;           // reactive（関数で包む）
> html`<div style=${() => ({ color: c() })}></div>`;
> ```
>
...
第1引数は**配列を返す accessor**（signal の読み `items`、または `() => items()` 等の派生）。
...
- **同じ key で中身が新しいオブジェクトに差し替わった更新**
  （`items().map(x => ({ ...x, done: true }))` のような immutable 更新）でも、行内の穴が更新される。
...
const list = html`<ul>
  ${For(
    items,                  // 配列を返す accessor（() => items() などの派生でも可）
    item => item.id,
...
第1引数は**値を返す accessor**（signal の読み `loggedIn`、または `() => loggedIn()` 等の派生）。
...
const view = html`<div>
  ${Show(
    loggedIn,               // 値を返す accessor（() => loggedIn() などの派生でも可）
    () => html`<p>ようこそ</p>`,
    () => html`<p>ログインしてください</p>`,
  )}
</div>`;
...
**基本は子の関数穴でよい。** `${() => list().map(...)}` でリスト、
`${() => cond() ? a : b}` で条件分岐が、コンポーネントを足さずに書ける。
...
`setup` は文脈オブジェクト `ctx` を1つ受け取る。`ctx.host`（要素自身）と、外部からの入力を
読む `ctx.prop` / `ctx.attr` が入っているので、必要なものを分割代入で取り出して使う。

**入力 → signal**: 外部からの入力は経路ごとに値の型が違うので、口を2つに分けている。どちらも
`signal`（`[読み, 書き]` のタプル）を返し、同じ name には同じ signal を返す。

- **`ctx.prop(name, initial?)` — プロパティ代入**（`Signal<T>`）。host に accessor を張るので
  `el.name = v` がそのまま signal に入る。オブジェクト・配列などリッチな値もそのまま `T` で通る
  （`html` の `.foo=${...}` のリッチな値もこの経路で届く）。upgrade 前（接続前）に代入されていた
  値も初期値として拾う。初期値の優先順は「upgrade 前のプロパティ > `initial`」。
- **`ctx.attr(name, initial?)` — 属性**（`Signal<string | null>`）。`MutationObserver` で観測し、
  変更を**文字列のまま** signal に流す（属性削除は `null`）。静的 HTML の `name="..."` は初期値
  として読む。初期値の優先順は「静的 HTML の属性 > `initial`（既定 `null`）」。属性は素で常に
  文字列なので、数値などが欲しければ**読む側で変換する**。setter は**属性へ書き戻す**
  （`setAttribute`、`null` で `removeAttribute`）ので、DOM 属性を単一の真実にできる。
...
// 数値が欲しければ読む側で変換する（属性は常に文字列）
defineElement("x-counter", ({ attr }) => {
  const [count] = attr("count", "0");
  return html`<p>${() => Number(count() ?? 0) + 1}</p>`;
});
// <x-counter count="41"> → 42

// リッチな値（配列・オブジェクト）はプロパティで受ける
...
属性・プロパティのどちらで書かれても1つの値として扱いたい場合は、`prop` と `attr` を
利用者側で `effect` で束ねる（型が違う2経路を1本化するのは利用者の責務にしている）。
setter の書き戻しは口ごとに違う: `attr` の setter は**属性へ書き戻す**（`setAttribute`、`null` で
`removeAttribute`）ので `x-el[foo] { ... }` の属性セレクタも効く。`prop` の setter は
プロパティ（＝ signal 自身）を更新するだけで属性には触れない。
...
defineElement("x-toggle", ({ host }) => {
  ...
  const toggle = () => {
    setOpen(!open());
    host.dispatchEvent(new CustomEvent("toggle", { detail: open() })); // 外向きに通知
  };
  return html`<button onClick=${toggle}>${() => (open() ? "閉じる" : "開く")}</button>`;
});
```
...
effect(() => host.toggleAttribute("open", open()));
```
...
```
src/    reactive.ts / node.ts / html.ts / for.ts / show.ts /
        element.ts / index.ts

...
| 優先 | 項目 | 理由 |
|---|---|---|
| 中 | #48, #49 emit / orchestrator のギャップ | ハイドレーション本体は実装済み。`Node` 子・ネスト要素が emit に乗らず、実用 SSR はここが律速。さらに進めるなら最優先（`For` / `Show` の emit は対応済み） |
| 中 | #42, #51, #53 [検討] の方針決定 | 実装より先に「やるか否か」を決める必要があり、他の作業（SSR スコープ・API 一本化）の前提になる |
| 低 | #45, #46, #52 SSR の周辺強化 | state 直列化・slot 対応・mismatch 検出。emit の穴（#48〜#49）が埋まってから／必要になってから |

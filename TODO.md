...
| 中 | #47, #48, #49 emit / orchestrator のギャップ | adopt 本体は実装済み（ただし #54 の穴あり）。`For` / `Show` / `Node` 子・ネスト要素が emit に乗らず、実用 SSR はここが律速。さらに進めるなら最優先 |
...
ハイドレーション本体なので、emit の穴（#47〜#49）より優先度は高い。

...
### 6. トップレベルの effect は手動解放 [割り切り]

親がいない場所で作った effect は自動では畳まれない（README にも明記）。
...
SVG なし（#9）は `html` にも同様に当てはまる。`setAttr` / `toNode` は `node.ts` に
共通化済みなので、対応の際はそこに入れれば h.ts / html.ts の両方にまとめて効く
（`ref` は `node.ts` の `isRef` 経由で h.ts / html.ts の両方に効かせ済み）。

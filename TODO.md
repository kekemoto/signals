...
### 50. ハイドレーション実装に合わせてドキュメントを更新 [未実装]

実装（`emit` / `hydration` / `defineElement` の adopt モード）が先行し、ドキュメントが追いついて
いない。

- **README.md** — ハイドレーションの現状と書き方を追記（`emit` + host の `data-hydrate` +
  `defineElement` の adopt、公開エントリ `hydrate`）。DOM API 節の後あたりに置く。
- **docs/ssr-hydration-plan.md** — 冒頭の「実装はまだ無い」が古い。stage 1〜4 実装済みの現状に
  合わせ、「設計方針」から「実装済みの構成 + 残課題（#45〜#49）」へ更新する。
- **TODO.md** — 本ファイルも実装に合わせて随時更新（#23 は更新済み。今後の差分を反映する）。

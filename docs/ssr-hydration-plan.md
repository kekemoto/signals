...
いまは「どう作るか」の方針・根拠に加えて、「どのファイルがどの役割を担っているか」「何が残課題か」を
記録する位置づけ。
...
**残課題**（TODO.md と対応）: ⑤state 直列化（#45）、slot × ハイドレーション（#46）、`emit` の
`For` / `Show` シリアライズ（#47）、`Node` 子のシリアライズ（#48）、ネストしたカスタム要素を辿る
サーバ orchestrator（`renderToString` 相当・#49）、mismatch 検出の強化（#52）。
...
検討の結論として、**真のハイドレーションをやる場合の最小構成**は次の組み合わせ。
...
これにより、当初懸念していた「DOM 構築版とサーバ文字列版でテンプレ解釈が二重実装になる」
問題が解消する。残るのは wire パス（＝文字列に焼けないクロージャ / effect を張る不可避の一手）
だが、これはエミッタの複製ではなく補完であり、しかも create と hydrate を統一する。
